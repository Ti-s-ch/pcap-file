...
use crate::pcapng::blocks::{
    EnhancedPacketBlock, InterfaceDescriptionBlock, InterfaceStatisticsBlock, NameResolutionBlock,
...
impl<'a> Block<'a> {
    ...
    pub(crate) fn from_reader<R: Read, B: ByteOrder>(
        ...
        if type_ == BlockType::SectionHeader {
            ...
            let endianness = match magic {
                ...
                _ => {
                    return Err(PcapError::InvalidField(
                        "SectionHeaderBlock: invalid magic number",
                    ))
                }
            };
            ...
            if initial_len != trailer_len {
                return Err(PcapError::InvalidField(
                    "Block initial_length != trailer_length",
                ));
            }
        ...
        if type_ == BlockType::SectionHeader {
            ...
            if slice.len() < initial_len as usize - 8 {
                return Err(PcapError::IncompleteBuffer(
                    initial_len as usize - 8 - slice.len(),
                ));
            }
    ...
    pub fn parsed(&self) -> Result<ParsedBlock, PcapError> {
        match self.endianness {
            Endianness::Big => {
                ParsedBlock::from_slice::<BigEndian>(self.type_, &self.body).map(|r| r.1)
            }
            Endianness::Little => {
                ParsedBlock::from_slice::<LittleEndian>(self.type_, &self.body).map(|r| r.1)
            }
        }
    }
}
...
impl<'a> ParsedBlock<'a> {
    ...
    pub fn from_slice<B: ByteOrder>(
        ...
        match type_ {
            ...
            _ => Ok((
                slice,
                ParsedBlock::Unknown(UnknownBlock::new(type_, slice.len() as u32, slice)),
            )),
        }
    }
...
pub(crate) fn opts_from_slice<'a, B, F, O>(
    mut slice: &'a [u8],
    func: F,
) -> Result<(&'a [u8], Vec<O>), PcapError>
where
    B: ByteOrder,
    F: Fn(&'a [u8], u16, u16) -> Result<O, PcapError>,
{
    ...
    loop {
        ...
        if slice.len() < length + pad_len {
            return Err(PcapError::InvalidField(
                "Option: length + pad.len() > slice.len()",
            ));
        }
