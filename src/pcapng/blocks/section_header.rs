    ...
    pub fn from_slice(mut slice: &'a [u8]) -> Result<(&'a [u8], Self), PcapError> {
        if slice.len() < 16 {
            return Err(PcapError::InvalidField(
                "SectionHeaderBlock: block length < 16",
            ));
        }
        ...
        fn parse_inner<B: ByteOrder>(
            mut slice: &[u8],
        ) -> Result<(&[u8], u16, u16, i64, Vec<SectionHeaderOption>), PcapError> {
    ...
    fn from_slice<B: ByteOrder>(slice: &'a [u8]) -> Result<(&'a [u8], Vec<Self>), PcapError> {
        opts_from_slice::<B, _, _>(slice, |slice, code, length| {
            let opt = match code {
                ...
                4 => {
                    SectionHeaderOption::UserApplication(Cow::Borrowed(std::str::from_utf8(slice)?))
                }

                2988 | 19372 => {
                    SectionHeaderOption::CustomUtf8(CustomUtf8Option::from_slice::<B>(code, slice)?)
                }
                2989 | 19373 => SectionHeaderOption::CustomBinary(
